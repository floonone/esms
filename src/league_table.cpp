...
void league_table::read_league_table_file(string filename) {
  ...
  if (table_in) {
    ...
    while (getline(table_in, line)) {
      ...
      add_new_team(
        name, played, won, drawn, lost, goals_for, goals_against, goal_difference, points);
    }
  }
}
...
//
void league_table::add_new_team(string name, int played, int won, int drawn, int lost,
  int goals_for, int goals_against, int goal_difference, int points) {
...
string league_table::dump_league_table(void) {
  ...
  for (map<string, team_data>::const_iterator i = teams.begin(); i != teams.end(); ++i) {
  ...
  for (vector<team_data>::const_iterator i = sorted_teams.begin(); i != sorted_teams.end(); ++i) {
    string line = format_str("%-4d %-21s %3d %4d %3d %3d %5d %4d %4d %5d\n",
      ...
      i->name.c_str(), i->played, i->won, i->drawn, i->lost, i->goals_for, i->goals_against,
      i->goal_difference, i->points);
