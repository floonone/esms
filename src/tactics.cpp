  ...
  for (line_i = mult_lines.begin(); line_i != mult_lines.end(); ++line_i) {
    ...
    if (tokens.size() == 5 && tokens[0] == "MULT") {
      if (!tactic_exists(tokens[1]))
        die("In tactics file: In line:\n%s\n%s - tactic doesn't exist", line_i->c_str(),
          tokens[1].c_str());

      if (!position_exists(tokens[2]))
        die("In tactics file: In line:\n%s\n%s - position doesn't exist", line_i->c_str(),
          tokens[2].c_str());

      if (!skill_exists(tokens[3]))
        die("In tactics file: In line:\n%s\n%s - skill doesn't exist", line_i->c_str(),
          tokens[3].c_str());
...
double tactics_manager::get_mult(
  const string tactic, const string opp_tactic, const string pos, const string skill) {
