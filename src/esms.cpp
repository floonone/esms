...
int main(int argc, char *argv[]) {
  ...
  if (opt->getValue("penalty_score")) {
    ...
    if (cup_flag == 1) {
      printf("\nScore: %s %d-%d %s", team[0].name, team[0].score, team[1].score, team[1].name);
...
void init_teams_data(teamsheet_reader teamsheet[2]) {
  ...
  for (l = 0; l <= 1; l++) {
    ...
    for (i = 1; i <= num_players; i++) {
      ...
      if (!strcmp(full_pos, "GK"))
        ...
        if (!is_legal_position(string(full_pos)))
          die("Illegal position %s of %s in %s's teamsheet", full_pos, team[l].player[i].name,
            team[l].name);
      ...
      //
      for (RosterPlayerIterator player = team[l].roster_players.begin();
           player != team[l].roster_players.end(); ++player) {
        ...
        //
        if (player->injury > 0)
          die("Player %s (%s) is injured", player->name.c_str(), team[l].name);

        if (player->suspension > 0)
          die("Player %s (%s) is suspended", player->name.c_str(), team[l].name);
      ...
      if (!found)
        die(
          "Player %s (%s) doesn't exist in the roster file", team[l].player[i].name, team[l].name);
    }
    ...
    if (pk_lines_tokens.size() == 2 && pk_lines_tokens[0] == "PK:") {
      ...
      if (i == 0)
        die("Error in penalty kick taker of %s, player %s not listed", team[l].name,
          pk_lines_tokens[1].c_str());
    } else {
  ...
  for (j = 0; j <= 1; j++) {
    ...
    for (i = 1; i <= num_players; i++) {
      team[j].player[i].tk_contrib = team[j].player[i].ps_contrib = team[j].player[i].sh_contrib =
        0;
    ...
    for (i = 1; i <= num_players; i++)
      for (k = 1; k <= num_players; k++) {
        if (k != i && !strcmp(team[j].player[i].name, team[j].player[k].name))
          die("Player %s (%s) is named twice in the team sheet", team[j].player[i].name,
            team[j].name);
      }
}
...
void change_tactic(int a, const char *newtct) {
  if (strcmp(newtct, team[a].tactic)) {
    ...
    fputs(the_commentary()
            .rand_comment(
              "CHANGETACTIC", minute_str().c_str(), team[a].name, team[a].name, team[a].tactic)
            .c_str(),
      comm);
  }
}
...
void substitute_player(int a, int out, int in, string newpos) {
  ...
  if (team[a].player[out].active == 1 && team[a].player[in].active == 2 &&
    ...
    fputs(the_commentary()
            .rand_comment("SUB", minute_str().c_str(), team[a].name, team[a].player[in].name,
              team[a].player[out].name, newpos.c_str())
            .c_str(),
      comm);
  }
}
...
void change_position(int a, int b, string newpos) {
  ...
  if (b != team[a].current_gk && team[a].player[b].active == 1) {
    ...
    if (pos_and_side2fullpos(team[a].player[b].pos, team[a].player[b].side) != newpos) {
      fputs(the_commentary()
              .rand_comment("CHANGEPOSITION", minute_str().c_str(), team[a].name,
                team[a].player[b].name, newpos.c_str())
              .c_str(),
        comm);
...
void random_injury(int a) {
  ...
  if (randomp((1500 + team[!a].aggression) / 50)) /* If someone got injured */
  {
    ...
    fprintf(comm, "%s",
      the_commentary()
        .rand_comment("INJURY", minute_str().c_str(), team[a].name, team[a].player[injured].name)
        .c_str());

    report_event *an_event = new report_event_injury(
      team[a].player[injured].name, team[a].name, formal_minute_str().c_str());
...
void calc_player_contributions(int a, int b) {
  if (team[a].player[b].active == 1 && team[a].current_gk != b) {
    double tk_mult =
      tact_manager().get_mult(team[a].tactic, team[!a].tactic, team[a].player[b].pos, "TK");
    double ps_mult =
      tact_manager().get_mult(team[a].tactic, team[!a].tactic, team[a].player[b].pos, "PS");
    double sh_mult =
      tact_manager().get_mult(team[a].tactic, team[!a].tactic, team[a].player[b].pos, "SH");
    ...
    team[a].player[b].tk_contrib =
      tk_mult * side_factor * team[a].player[b].tk * team[a].player[b].fatigue;
    team[a].player[b].ps_contrib =
      ps_mult * side_factor * team[a].player[b].ps * team[a].player[b].fatigue;
    team[a].player[b].sh_contrib =
      sh_mult * side_factor * team[a].player[b].sh * team[a].player[b].fatigue;
  }
...
void adjust_contrib_with_side_balance(int a) {
  ...
  for (vector<string>::const_iterator pos = positions.begin(); pos != positions.end(); ++pos) {
    ...
    if (on_pos_left != on_pos_right) {
      double tax_ratio =
        0.25 * double(abs(on_pos_right - on_pos_left)) / (on_pos_right + on_pos_left);
...
void calc_shotprob(int a) {
  ...
  team[a].shot_prob =
    (double) 1.8 *
    (team[a].aggression / 50.0 +
      800.0 * (double) pow(((1.0 / 3.0 * team[a].team_shooting + 2.0 / 3.0 * team[a].team_passing) /
                             (team[!(a)].team_tackling + 1.0)),
                2));
...
void if_shot(int a) {
  ...
  if (randomp((int) team[a].shot_prob)) {
    ...
    if (randomp(7500)) {
      ...
      fprintf(comm, "%s",
        the_commentary()
          .rand_comment("ASSISTEDCHANCE", minute_str().c_str(), team[a].name,
            team[a].player[assister].name, team[a].player[shooter].name)
          .c_str());
      ...
      fprintf(comm, "%s",
        the_commentary()
          .rand_comment("CHANCE", minute_str().c_str(), team[a].name, team[a].player[shooter].name)
          .c_str());
    }

    chance_tackled = (int) (4000.0 * ((team[!a].team_tackling * 3.0) /
                                       (team[a].team_passing * 2.0 + team[a].team_shooting)));
    ...
    if (randomp(chance_tackled)) {
      ...
      fprintf(
        comm, "%s", the_commentary().rand_comment("TACKLE", team[!a].player[tackler].name).c_str());
    } else /* Chance was not tackled, it will be a shot on goal */
    {
      fprintf(
        comm, "%s", the_commentary().rand_comment("SHOT", team[a].player[shooter].name).c_str());
      ...
      if (if_ontarget(a, shooter)) {
        ...
        if (if_goal(a, shooter)) {
          ...
          if (!is_goal_cancelled()) {
            ...
            report_event *an_event = new report_event_goal(
              team[a].player[shooter].name, team[a].name, formal_minute_str().c_str());
          ...
          fprintf(comm, "%s",
            the_commentary()
              .rand_comment("SAVE", team[!a].player[team[!a].current_gk].name)
              .c_str());
...
void if_foul(int a) {
  ...
  if (randomp((int) team[a].aggression * 3 / 4)) {
    ...
    fprintf(comm, "%s",
      the_commentary()
        .rand_comment("FOUL", minute_str().c_str(), team[a].name, team[a].player[fouler].name)
        .c_str());
    ...
    if ((fouler == team[a].current_gk) || (randomp(500))) {
      ...
      if (team[!a].player[team[!a].penalty_taker].active != 1 || team[!a].penalty_taker == -1) {
        ...
        for (int i = 1; i <= num_players; ++i) {
          if (team[!a].player[i].active == 1 &&
              team[!a].player[i].sh * team[!a].player[i].fatigue > max) {
      ...
      fprintf(comm, "%s",
        the_commentary()
          .rand_comment("PENALTY", team[!a].player[team[!a].penalty_taker].name)
          .c_str());
      ...
      if (randomp(8000 + team[!a].player[team[!a].penalty_taker].sh * 100 -
        ...
        team[a].player[team[a].current_gk].conceded++;
        fprintf(comm, "\n          ...  %s %d-%d %s...", team[0].name, team[0].score, team[1].score,
          team[1].name);

        report_event *an_event = new report_event_penalty(
          team[!a].player[team[!a].penalty_taker].name, team[!a].name, formal_minute_str().c_str());
        report_vec.push_back(an_event);
      ...
      } else /* If the penalty taker didn't score */
      {
        ...
        if (randomp(7500))
          fprintf(comm, "%s",
            the_commentary().rand_comment("SAVE", team[a].player[team[a].current_gk].name).c_str());
...
void bookings(int a, int b, int card_color) {
  if (card_color == YELLOW) {
    ...
    if (team[a].player[b].yellowcards == 2) {
      ...
      report_event *an_event = new report_event_red_card(
        team[a].player[b].name, team[a].name, formal_minute_str().c_str());
      report_vec.push_back(an_event);
    ...
    report_event *an_event =
      new report_event_red_card(team[a].player[b].name, team[a].name, formal_minute_str().c_str());
    report_vec.push_back(an_event);
...
void print_final_stats(void) {
  ...
  fprintf(comm, "\n\n%-22s: %s %2d %s %d",
    the_commentary().rand_comment("COMM_SHOTSOFFTARGET").c_str(), team[0].name,
    team[0].finalshots_off, team[1].name, team[1].finalshots_off);

  fprintf(comm, "%-22s: %s %2d %s %d", the_commentary().rand_comment("COMM_SHOTSONTARGET").c_str(),
    team[0].name, team[0].finalshots_on, team[1].name, team[1].finalshots_on);

  fprintf(comm, "\n%-22s: %s %2d %s %d\n", the_commentary().rand_comment("COMM_SCORE").c_str(),
    team[0].name, team[0].score, team[1].name, team[1].score);
  ...
  for (int j = 0; j <= 1; j++) {
    ...
    fprintf(comm, "\nName          Pos Prs St Tk Ps Sh Sm | Min Sav Ktk Kps Ass Sht Gls Yel Red "
                  "Inj KAb TAb PAb SAb Fit");
    fprintf(comm, "\n------------------------------------------------------------------------------"
                  "--------------------");
    // Totals
    int t_saves = 0, t_tackles = 0, t_keypasses = 0, t_assists = 0, t_shots = 0, t_goals = 0,
        t_yellowcards = 0, t_redcards = 0, t_injured = 0;
    ;

    // Print stats for each player and collect totals
    for (i = 1; i <= num_players; i++) {
      fprintf(comm,
        "\n%-13s %3s %3s%3d%3d%3d%3d%3d | %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d "
        "%3d",
        team[j].player[i].name,
        pos_and_side2fullpos(team[j].player[i].pos, team[j].player[i].side).c_str(),
        team[j].player[i].pref_side, team[j].player[i].st, team[j].player[i].tk,
        team[j].player[i].ps, team[j].player[i].sh, team[j].player[i].stamina,
        team[j].player[i].minutes, team[j].player[i].saves, team[j].player[i].tackles,
        team[j].player[i].keypasses, team[j].player[i].assists, team[j].player[i].shots,
        team[j].player[i].goals, team[j].player[i].yellowcards, team[j].player[i].redcards,
        team[j].player[i].injured, team[j].player[i].st_ab, team[j].player[i].tk_ab,
        team[j].player[i].ps_ab, team[j].player[i].sh_ab, int(team[j].player[i].fatigue * 100.0));
    ...
    fprintf(comm, "                                %3d %3d %3d %3d %3d %3d %3d %3d %3d\n", t_saves,
      t_tackles, t_keypasses, t_assists, t_shots, t_goals, t_yellowcards, t_redcards, t_injured);
  }
  ...
  if (team_stats_total_enabled) {
    ...
    for (i = 0; i < 10; ++i) {
      fprintf(comm, "\n%s    %2d    %6.2f   %6.2f   %6.2f", team[0].name, i * 10,
        teamStatsTotal[0][i][0], teamStatsTotal[0][i][1], teamStatsTotal[0][i][2]);

      fprintf(comm, "\n%s    %2d    %6.2f   %6.2f   %6.2f", team[1].name, i * 10,
        teamStatsTotal[1][i][0], teamStatsTotal[1][i][1], teamStatsTotal[1][i][2]);
    }
  }
}
...
void update_reports_file(string work_dir) {
  ...
  // Add the game score
  //
  fprintf(reportsfile, "\n%s %d - %d %s\n", team[0].fullname, team[0].score, team[1].score,
    team[1].fullname);
