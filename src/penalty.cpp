...
void TakePenalty(int nTeam, int nPenaltyNum) {
  fprintf(comm, "\n%s",
    the_commentary().rand_comment("PENALTY", PenaltyTaker[nTeam][nPenaltyNum].name).c_str());
  ...
  if (randomp(8000 + PenaltyTaker[nTeam][nPenaltyNum].sh * 100 -
    ...
    PenScore[nTeam]++;
    fprintf(comm, "\n          ...  %s %d-%d %s...", team[0].name, PenScore[0], PenScore[1],
      team[1].name);
  } else {
    ...
    if (rnd < 5)
      fprintf(comm, "%s",
        the_commentary()
          .rand_comment("SAVE", team[!nTeam].player[team[!nTeam].current_gk].name)
          .c_str());
    else
      fprintf(comm, "%s",
        the_commentary()
          .rand_comment("OFFTARGET", team[!nTeam].player[team[!nTeam].current_gk].name)
          .c_str());
  }
}
