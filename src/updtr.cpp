...
int main(int argc, char *argv[]) {
  ...
  if (opt->getArgc() == 1) {
    ...
    cout << "What would you like to do ?\n\n"
            ...
            "7)  Decrease suspensions, injuries + update rosters (50% fitness recovery), league "
            "table\n"
            "8)  Decrease suspensions, injuries + update rosters (100% fitness recovery), league "
            "table\n\n"
...
//
void handle_skill_change(
  string player_name, string team_name, string skill_name, int &ab_points, int &skill) {
  // Increase ?
  if (ab_points >= 1000) {
    ab_points -= 700;
    skill++;
    skill_change_report.push_back(the_commentary().rand_comment(
      "UPDTR_SKILL_INCREASE", player_name.c_str(), team_name.c_str(), skill_name.c_str()));
  }
  // Decrease ?
  else if (ab_points < 0) {
    ab_points += 300;
    skill--;
    skill_change_report.push_back(the_commentary().rand_comment(
      "UPDTR_SKILL_DECREASE", player_name.c_str(), team_name.c_str(), skill_name.c_str()));
  }
...
int calc_perf_points(
  int goals, int shots, int tackles, int saves, int assists, int keypasses, int dp) {
...
void update_rosters() {
  ...
  weekly_stats["goals"] = weekly_stats["goals_DF"] = weekly_stats["goals_DM"] =
    weekly_stats["goals_MF"] = weekly_stats["goals_AM"] = weekly_stats["goals_FW"] =
      weekly_stats["assists"] = weekly_stats["assists_DF"] = weekly_stats["assists_DM"] =
        weekly_stats["assists_MF"] = weekly_stats["assists_DM"] = weekly_stats["assists_FW"] =
          weekly_stats["yellows"] = weekly_stats["reds"] = weekly_stats["injuries"] = 0;
  ...
  while (getline(dir_file, line)) {
    ...
    for (int team_n = 0; team_n <= 1; ++team_n) {
      ...
      for (int player_n = 0; player_n < num_players; ++player_n) {
        RosterPlayerIterator player =
          get_player_by_name_from_roster(stats_teams[team_n][player_n].name, players);
        ...
        //
        handle_skill_change(player->name, team_name[team_n], "St", player->st_ab, player->st);
        handle_skill_change(player->name, team_name[team_n], "Tk", player->tk_ab, player->tk);
        handle_skill_change(player->name, team_name[team_n], "Ps", player->ps_ab, player->ps);
        handle_skill_change(player->name, team_name[team_n], "Sh", player->sh_ab, player->sh);
        ...
        if ((player->dp / suspension_margin) < (dp_after_update / suspension_margin)) {
          ...
          if (player->suspension == 1)
            comm_line = the_commentary().rand_comment(
              "UPDTR_SUSPENDED_1", player->name.c_str(), team_name[team_n].c_str());
          else
            comm_line = the_commentary().rand_comment("UPDTR_SUSPENDED_N", player->name.c_str(),
              team_name[team_n].c_str(), player->suspension);
        ...
        if (stats_teams[team_n][player_n].injured) {
          ...
          if (player->injury == 0)
            comm_line = the_commentary().rand_comment(
              "UPDTR_INJURY_NONE", player->name.c_str(), team_name[team_n].c_str());
          else if (player->injury == 1)
            comm_line = the_commentary().rand_comment(
              "UPDTR_INJURY_1", player->name.c_str(), team_name[team_n].c_str());
          else if (player->injury <= 4)
            comm_line = the_commentary().rand_comment("UPDTR_INJURY_LIGHT", player->name.c_str(),
              team_name[team_n].c_str(), player->injury);
          else
            comm_line = the_commentary().rand_comment(
              "UPDTR_INJURY_HARD", player->name.c_str(), team_name[team_n].c_str(), player->injury);
        ...
        int perf_points = calc_perf_points(stats_teams[team_n][player_n].goals,
          stats_teams[team_n][player_n].shots, stats_teams[team_n][player_n].tackles,
          stats_teams[team_n][player_n].saves, stats_teams[team_n][player_n].assists,
          stats_teams[team_n][player_n].keypasses, stats_teams[team_n][player_n].dp);
  ...
  stats_report.push_back(
    format_str("Goals:        %3d  (DFs - %d, DMs - %d, MFs - %d, AMs - %d, FWs - %d)",
      weekly_stats["goals"], weekly_stats["goals_DF"], weekly_stats["goals_DM"],
      weekly_stats["goals_MF"], weekly_stats["goals_AM"], weekly_stats["goals_FW"]));
  stats_report.push_back(
    format_str("Assists:      %3d  (DFs - %d, DMs - %d, MFs - %d, AMs - %d, FWs - %d)",
      weekly_stats["assists"], weekly_stats["assists_DF"], weekly_stats["assists_DM"],
      weekly_stats["assists_MF"], weekly_stats["assists_AM"], weekly_stats["assists_FW"]));
...
//
void transform_all_players(
  void (*transformer_proc)(RosterPlayerIterator &, string, void *), void *arg) {
...
void transformer_reset_stats(RosterPlayerIterator &player, string team_name, void *arg) {
  player->games = player->saves = player->tackles = player->keypasses = player->shots =
    player->goals = player->assists = player->dp = 0;
  player->fitness = 100;
...
void transformer_decrease_sus_inj(RosterPlayerIterator &player, string team_name, void *arg) {
  ...
  if (*inj_sus_flag & SUSPENSIONS) {
    ...
    if (player->suspension == 0)
      suspension_report.push_back(the_commentary().rand_comment(
        "UPDTR_END_SUSPENSION", player->name.c_str(), team_name.c_str()));
  ...
  if (*inj_sus_flag & INJURIES) {
    ...
    if (player->injury == 0) {
      injury_report.push_back(
        the_commentary().rand_comment("UPDTR_END_INJURY", player->name.c_str(), team_name.c_str()));
...
void make_leaders_report(const vector<player_stat> &list, string heading, string stat) {
  ...
  for (int i = 0; i < min<int>(15, list.size()); ++i) {
    ...
    int the_stat = (stat == "Gls")   ? list[i].goals
                   : (stat == "Ass") ? list[i].assists
                   : (stat == "DPs") ? list[i].dp
                                     : list[i].perf_points;

    leaders_report.push_back(
      format_str("%-20s %5d  %5d", name_and_team.c_str(), list[i].games, the_stat));
  }
}
...
bool leaders_predicate_perf(const player_stat &p1, const player_stat &p2) {
  return (p1.perf_points > p2.perf_points) ||
         (p1.perf_points == p2.perf_points && p1.games < p2.games);
}
...
void generate_leaders(void) {
  ...
  while (getline(dir_file, line)) {
    ...
    for (RosterPlayerIterator player = players.begin(); player != players.end(); ++player) {
      int perf_points = calc_perf_points(player->goals, player->shots, player->tackles,
        player->saves, player->assists, player->keypasses, player->dp);

      stat_players.push_back(player_stat(player->name, team_name, player->games, player->goals,
        player->assists, player->dp, perf_points));
