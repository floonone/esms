...
///
string choose_best_player(const RosterPlayerArray &players, const set<string> &chosen_players,
  int (*skill)(RosterPlayerConstIterator player)) {
  int best_skill = -1;
  string name_of_best = "";

  for (RosterPlayerConstIterator player = players.begin(); player != players.end(); ++player) {
    if (chosen_players.find(player->name) == chosen_players.end() && skill(player) > best_skill &&
