...
char nationalities[20][4] = {"arg", "aus", "bra", "bul", "cam", "cro", "den", "eng", "fra", "ger",
  "hol", "ire", "isr", "ita", "jap", "nig", "nor", "saf", "spa", "usa"};
...
int main(int argc, char *argv[]) {
  ...
  for (int roster_count = 1; roster_count <= cfg_n_rosters; ++roster_count) {
    ...
    sort(players_arr.begin(), players_arr.begin() + cfg_n_gk, more_st);

    sort(players_arr.begin() + cfg_n_gk, players_arr.begin() + cfg_n_gk + cfg_n_df + cfg_n_dm,
      more_tk);

    sort(players_arr.begin() + cfg_n_gk + cfg_n_df + cfg_n_dm,
      players_arr.begin() + cfg_n_gk + cfg_n_df + cfg_n_dm + cfg_n_mf + cfg_n_am, more_ps);

    sort(players_arr.begin() + cfg_n_gk + cfg_n_df + cfg_n_dm + cfg_n_mf + cfg_n_am,
      players_arr.end(), more_sh);
...
string gen_random_name(void) {
  ...
  string consonantish_not_begin = "mn,nh,rt,rs,rst,dn,nd,ds,bt,bs,bl,sk,vr,ks,sy,ny,vr,sht,ck";
