...
string write_roster_players(string roster_filename, const RosterPlayerArray &players_arr) {
  ...
  rosterfile << "Name         Age Nat Prs St Tk Ps Sh Sm Ag KAb TAb PAb SAb Gam Sav Ktk Kps Sht "
                "Gls Ass  DP Inj Sus Fit\n";
  rosterfile << "----------------------------------------------------------------------------------"
                "--------------------\n";

  for (RosterPlayerConstIterator player = players_arr.begin(); player != players_arr.end();
       ++player) {
    rosterfile << format_str(
      "%-13s%3d%4s%4s%3d%3d%3d%3d%3d%3d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d\n",
      player->name.c_str(), player->age, player->nationality.c_str(), player->pref_side.c_str(),
      player->st, player->tk, player->ps, player->sh, player->stamina, player->ag, player->st_ab,
      player->tk_ab, player->ps_ab, player->sh_ab, player->games, player->saves, player->tackles,
      player->keypasses, player->shots, player->goals, player->assists, player->dp, player->injury,
      player->suspension, player->fitness);
  }
